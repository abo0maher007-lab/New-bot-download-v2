...
async def download_with_ytdlp(url: str, output_template: str):
    """استخدام yt-dlp مع إعدادات تفادي حظر 403 وحماية المتصفحات"""
    cmd = [
        "yt-dlp",
...
async def download_with_curl_cffi(url: str, filepath: str):
    """التحميل عبر curl_cffi بتزييف بصمة TLS لتجاوز Cloudflare وHTTP 403"""
    try:
...
async def url_handler(client, message: Message):
    ...
    # المحاولة 1: استخدام curl_cffi لتخطي حظر 403 وبصمة TLS
    ...
    # المحاولة 2: استخدام yt-dlp المحسنة
